...
$peq = new processEarthquakes($file);
...
class processEarthquakes
{
	...
	public function __construct($file)
	{
		...
		$this->getEarthquakes($store, $notify, $file);
	}

	public function getEarthquakes($store, $notify, $file)
	{
//		$this->_logger->error(time());
		...
		$this->_logger->debug('Checking a file for earthquakes!');

		$earthquakeArray = json_decode(readfile($file));

		foreach ($earthquakeArray as $earthquakeElement) {

			echo $earthquakeElement;
			continue;
